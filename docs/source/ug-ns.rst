...
The user needs to provide the path to the HSQC experiment to analyze through the parameters ``basedir`` and ``expno``. 
...
The calculation is performed for each entry of the ``--xred`` argument.
